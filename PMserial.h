...
public:
  union {
    uint16_t data[12]; // all PM/NC data
    struct {
      uint16_t pm[6]; // particulate matter [ug/m3]
      uint16_t nc[6]; // number concentration [#/100cc]
    };
    struct {
      // pmX [ug/m3]: PM1.0, PM2.5 & PM10
      uint16_t pm01_sp, pm25_sp, pm10_sp, pm01_sa, pm25_sa, pm10_sa;
...
protected:
  ...
  // message timing
  static const uint16_t max_wait_ms = 300;
